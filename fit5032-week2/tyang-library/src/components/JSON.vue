    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
        >
        <span :class="{ highlight1: author.name === 'George Orwell' }">{{ author.name }}</span> ({{ author.birthYear }})
        </li>
      </ul>
      ...
      <li v-for="work in orwell?.famousWorks" :key="work">{{ work.title }} ({{ work.year }})</li>
    </section>
...
const orwell = computed(() => {
  ...
  return authors.find((a) => a.name == 'Jane Austen')
})
